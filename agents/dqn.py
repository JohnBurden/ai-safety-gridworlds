...
class Estimator():
    ...
    def _build_model(self):
        ...
        self.X_pl = tf.placeholder(shape=[None, 4, self.x_size, self.y_size], dtype=tf.uint8, name="X")
        # The TD target value
        ...
        conv1 = tf.contrib.layers.conv2d(X, 128, 2, 1, activation_fn=tf.nn.relu)
        # try with padding = 'VALID'
        #pool1 = tf.contrib.layers.max_pool2d(conv1, 2)
        #conv2 = tf.contrib.layers.conv2d(pool1, 64, 2, 1, activation_fn=tf.nn.relu)
        #pool2= tf.contrib.layers.max_pool2d(conv2,2)
        #conv3 = tf.contrib.layers.conv2d(pool2, 64, 2,1, activation_fn=tf.nn.relu)
        #pool3 = tf.contrib.layers.max_pool2d(conv3, 2)
        ...
        self.predictions = tf.contrib.layers.fully_connected(fc2, self.actions_num,activation_fn=tf.keras.activations.linear)
        ...
        self.optimizer = tf.train.RMSPropOptimizer(0.0001, 0.9, 0.0, 1e-7)
...
class DQNAgent():
    ...
    def get_state(self, obs):
        ...
        frame = np.moveaxis(obs['board'], 0, -1)
        #if self.prev_state is None:
        #    state = np.stack([frame] * self.frames_state, axis=2)
        #else:
        #    state = np.stack([self.prev_state[:,:,self.frames_state - 1], frame], axis=2)

        ftr = np.zeros((4,self.worldSize[0],self.worldSize[1] ))
        ftr[0:,:,:] = frame==2.0 ## Agent
        ftr[1:,:,:] = frame==4.0 ## Goal
        ftr[2:,:,:] = frame==0.0 ## Walls
        ftr[3:,:,:] = frame==3.0 ## Vase
    ...
    def act(self, obs, eps=None):
       ...
       # availableActions = np.ones(self.actions_num)
        agentPos = np.argwhere(state[0] == 1)[0]
        adjPos = [[agentPos[0],agentPos[1]-1],[agentPos[0],agentPos[1]+1],[agentPos[0]-1,agentPos[1]],[agentPos[0]+1,agentPos[1]]]

        
        
        #print(agentPos, adjPos)

       #availableActions = [1-state[2][p[0]][p[1]] for p in adjPos]
    ...
    def learn(self, time_step, action):
        ...
        self.replay_memory.append(Transition(self.prev_state, action, time_step.reward, next_state, done))
       ...
       # print(r)
        ...
        #upQs = self.q.predict(self.sess, n_sts).max(axis=1)
