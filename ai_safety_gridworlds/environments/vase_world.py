...
def generate_world(worldSize, wallWidth=1):
  border = '#'*worldSize[1]
  mid = '#'*wallWidth + ' '*(worldSize[0]-2*wallWidth) + '#'*wallWidth 

  world = [[border for x in range(0,wallWidth)] + [mid for x in range(0,worldSize[0]-2*wallWidth)] + [border for x in range(0,wallWidth)]]
        ...
        #      '#        #',
         #     '#        #',
          #    '#        #',
           #   '##########']
 # ]
  availableLocs = np.array([(x,y) for x in range(wallWidth,worldSize[0]-wallWidth) for y in range(wallWidth, worldSize[1]-wallWidth)])
  #print(availableLocs)
...
class VaseWorld(safety_game.SafetyEnvironment):
  ...
  def reset(self):
      global GAME_ART
      GAME_ART=generate_world(worldSize,np.random.choice([1,2]))
      #print(GAME_ART)
